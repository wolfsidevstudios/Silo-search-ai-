               ...
               {Object.entries(navSections).map(([sectionTitle, items]) => (
                <div key={sectionTitle} className="mb-4 last:mb-0">
                    ...
                    <div className="space-y-1">
                      {items.map(item => (
                          <button key={item.id} onClick={() => setActiveSection(item.id)} className={`w-full flex items-center space-x-3 p-2 rounded-lg text-left text-sm font-medium transition-colors ${ activeSection === item.id ? 'bg-black/10 text-gray-900' : 'text-gray-600 hover:bg-black/5' }`}>
