import React from 'react';

interface NavigationTabsProps {
  activeTab: 'search' | 'discover' | 'create' | 'ai-labs' | 'spaces';
  onNavigate: (path: string) => void;
}

export const NavigationTabs: React.FC<NavigationTabsProps> = ({ activeTab, onNavigate }) => (
    <div className="mb-8 flex space-x-1 bg-gray-200 p-1 rounded-full">
        <button
            onClick={() => onNavigate('/search')}
            className={`px-6 py-2 text-sm font-semibold rounded-full transition-colors ${activeTab === 'search' ? 'bg-white shadow' : 'text-gray-600 hover:bg-white/50'}`}
        >
            Search
        </button>
        <button
            onClick={() => onNavigate('/discover')}
            className={`px-6 py-2 text-sm font-semibold rounded-full transition-colors ${activeTab === 'discover' ? 'bg-white shadow' : 'text-gray-600 hover:bg-white/50'}`}
        >
            Discover
        </button>
        <button
            onClick={() => onNavigate('/ai-labs')}
            className={`px-6 py-2 text-sm font-semibold rounded-full transition-colors ${activeTab === 'ai-labs' ? 'bg-white shadow' : 'text-gray-600 hover:bg-white/50'}`}
        >
            AI Labs
        </button>
        <button
            onClick={() => onNavigate('/spaces')}
            className={`px-6 py-2 text-sm font-semibold rounded-full transition-colors ${activeTab === 'spaces' ? 'bg-white shadow' : 'text-gray-600 hover:bg-white/50'}`}
        >
            Spaces
        </button>
        <button
            onClick={() => onNavigate('/create')}
            className={`px-6 py-2 text-sm font-semibold rounded-full transition-colors ${activeTab === 'create' ? 'bg-white shadow' : 'text-gray-600 hover:bg-white/50'}`}
        >
            Create
        </button>
    </div>
);