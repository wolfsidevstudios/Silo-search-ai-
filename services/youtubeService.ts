...
export async function fetchTrendingYouTubeShorts(apiKey: string): Promise<YouTubeVideo[]> {
  if (!apiKey) {
    console.warn("YouTube API key is missing. Skipping trending shorts search.");
    return [];
  }

  const endpoint = `https://www.googleapis.com/youtube/v3/search?part=snippet&q=%23shorts&key=${apiKey}&type=video&maxResults=10&order=viewCount`;

  try {
    const response = await fetch(endpoint);
    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(`YouTube API error for shorts: ${errorData.error.message}`);
    }
